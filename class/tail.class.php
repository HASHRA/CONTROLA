					...
					function updateLog() {
						...
						$.getJSON('?ajax=1&tab=' + lastTab + '&lastsize=' + tab.data('lastSize') + '&grep='+tab.data('grep') + '&invert='+tab.data('invert'), function(data) {
							...
							$.each(data.data, function(key, value) { 
								// Gene add
								value=value.replace(/ +(?:\\{3,}|\|{3,}|\/{3,}|\-{3,})[\r\n]+/g,"");
								value=value.replace(/(.*?\(avg\).*)/g,'<font style="color:#d44950;">$1</font>');
								value=value.replace(/(.*?yay\!\!\!.*)/g,'<font style="color:#77ff00;">$1</font>');
								value=value.replace(/(\033\[1;31m)/g,'<font style="color:#ff0000;">');
								value=value.replace(/(\033\[1;32m)/g,'<font style="color:#4f9fcf;">');
								value=value.replace(/(\033\[1;33m)/g,'<font style="color:#ffdd55;">');
								value=value.replace(/(\033\[1;34m)/g,'<font style="color:#0000aa;">');
								value=value.replace(/(\033\[0m)/g,'</font>');
								value=value.replace(/buf\[\d+\]\s+before\s+swab\s+[0-9a-f]+/g,'');
								value=value.replace(/buf\[\d+\]\s+after\s+swab\s+[0-9a-f]+/g,'');
								value=value.replace(/[\r]{2,}/g,'');
								$(".results", tab).append('<span style="color:#999;display:block;">'+value+'</span>');
							});
							if(scroll) {
								scrollToBottom();
							}
